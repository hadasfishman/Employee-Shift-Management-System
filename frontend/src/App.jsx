...
function App() {
  ...
  // Dynamic Employee Roster with Hierarchical Ranks (1=Junior, 2=Senior, 3=Team Lead)
  ...
  // 3. Scheduling History Configuration
  ...
  // Generate and download a blank Excel template for importing employees
  const downloadTemplate = () => {
    const templateRows = [
      { 'שם': 'John Doe', 'דרגה': 3 },
      { 'שם': 'Jane Smith', 'דרגה': 2 },
      { 'שם': 'Alex Johnson', 'דרגה': 1 }
    ];
  ...
  // Parse uploaded Excel file and update employee state
  const handleFileUpload = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        }).filter(emp => emp.name && emp.name !== 'undefined' && emp.name !== 'John Doe' && emp.name !== 'Jane Smith' && emp.name !== 'Alex Johnson');

        if (parsedEmployees.length === 0) throw new Error("No valid employee data found in the file.");
        setEmployees(parsedEmployees);
        alert(`Successfully loaded ${parsedEmployees.length} employees from file!`);
      } catch (err) {
  ...
  // Handle manual custom shift additions
  ...
  // Main HTTP POST request to the Optimization Engine (Backend)
  const generateSchedule = async (forcedSeed = null) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Server returned an optimization calculation error.');
      
      const data = await response.json();
      if (data.status === 'success') setSchedule(data.schedule);
      else throw new Error(data.message || 'Failed to calculate schedule.');
    } catch (err) {
  ...
  // 🔒 Lock and approve the current schedule, saving it to the SQLite database
  const saveScheduleToDB = async () => {
    ...
    try {
      ...
      if (data.status === 'success') {
        setSaveSuccessMessage('✅ Schedule officially locked and saved to the DB! It will serve as history for future runs.');
      } else {
        alert('Save Error: ' + data.message);
      }
    } catch (err) {
      alert('Failed to communicate with the server for saving: ' + err.message);
    }
  };

  // Export current active schedule to an Excel file
  const exportToExcel = () => {
    if (!schedule) return;
    const excelRows = schedule.map(item => {
      // Sort assigned staff hierarchically: Team Lead (3) -> Senior (2) -> Junior (1)
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 font-sans p-8" dir="rtl">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Employee Roster Management Panel */}
        ...
        <div className="bg-gray-800 p-6 rounded-xl shadow-lg mb-8 border border-gray-700">
          ...
          {/* 3. Scheduling History Selection */}
